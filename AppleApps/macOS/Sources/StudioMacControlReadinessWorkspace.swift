...
struct StudioMacControlReadinessPage: View {
    ...
    private var detailGrid: some View {
        LazyVGrid(columns: [GridItem(.adaptive(minimum: 360), spacing: 16)], spacing: 16) {
            StudioControlBridgeLauncherHealthView(snapshot: bridgeSnapshot)
            StudioControlBridgeServerDiscoveryView(snapshot: bridgeSnapshot)
            StudioControlBridgeWorkspaceSwitcherView(snapshot: bridgeSnapshot, loadSupportedApp: loadSupportedApp)
            StudioControlBridgeFileRecoveryView(snapshot: bridgeSnapshot, loadSupportedApp: loadSupportedApp)
            StudioControlBridgeDiffArtifactView(snapshot: bridgeSnapshot)
            StudioControlBridgeStructuredDraftInboxView(snapshot: bridgeSnapshot)
            StudioControlBridgeRepoPreflightView(snapshot: bridgeSnapshot)
            StudioControlBridgeSafeApplyWizardView(snapshot: bridgeSnapshot)
            StudioControlBridgeVisualSmokeView(snapshot: bridgeSnapshot)
...
struct StudioControlReadinessPack: Decodable {
    let schema: String
...
extension StudioControlReadinessPack {
...
struct StudioControlReadinessRow: View {
    let title: String
    let subtitle: String
    let status: String
...
struct StudioControlStatusBadge: View {
    let status: String
...
enum StudioControlReadinessTone {
